...
Ancient and liturgical Greek use it for a different reason. A Modern Greek
voice is the right one -- that is how the Koine text is actually chanted --
but it only knows monotonic orthography and **silently drops a word-initial
vowel carrying a breathing mark**: `ἁγιασθήτω` and `γιασθήτω` synthesise to
byte-identical audio. Seventeen of the twenty-one such words in the NA28 text
lost their first letter before this was fixed. The `.say` lines carry the same
words in monotonic orthography, generated by stripping breathings and iota
subscripts, folding circumflex and grave to acute, then dropping the accent
from monosyllables. Do not hand-transcribe that; a syllable counter must count
*vowel runs in the word*, or `ἐπὶ` comes out as an unaccented monosyllable.

If a Greek reading ever sounds like it is swallowing syllables, synthesise a
word and the same word minus its first letter and compare the audio. Identical
output is the tell.
